...
function RadialDiagram({
  ...
  return (
    <div className="relative h-[240px] sm:h-[280px] flex items-center justify-center shrink-0">
      {/* Central AuraCore Node with Progress Ring */}
      <div className="absolute z-20 flex flex-col items-center justify-center w-20 h-20 sm:w-24 sm:h-24">
        <svg
        ...
        <motion.div
          ...
          className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-gradient-to-tr from-teal-500 to-blue-600 dark:from-cyan-400 dark:to-teal-400 p-0.5 shadow-[0_0_30px_rgba(34,211,238,0.4)] flex items-center justify-center relative z-10"
        >
        ...
        return (
          <button
            ...
            className={cn(
              ...
              isActive
                ? "border-cyan-400 bg-cyan-950/80 dark:bg-cyan-950/80 shadow-[0_0_20px_rgba(34,211,238,0.5)] scale-105 text-cyan-400"
                : "border-slate-800 bg-slate-900/50 dark:bg-slate-900/50 text-slate-400 opacity-60 scale-100",
              !isActive &&
      ...
      {RESEARCH_VECTORS.map((vector, index) => {
        ...
        return (
          <button
            ...
            onClick={() => onSelectVector(index)}
            className={cn(
              "flex items-center gap-2 px-3 py-2 rounded-xl border whitespace-nowrap shrink-0 transition-all duration-300",
              isActive
                ? "border-cyan-400 bg-cyan-950/80 shadow-[0_0_20px_rgba(34,211,238,0.5)] text-cyan-400 scale-105"
                : "border-slate-800 bg-slate-900/50 text-slate-400 opacity-60"
            )}
          >
            <IconComponent className="w-4 h-4" />
...
export function TargetingArchitecture() {
  ...
  useMotionValueEvent(scrollYProgress, "change", (progress) => {
    ...
    setActiveVector(progressToVectorIndex(progress));
      ...
      <div className="flex items-start justify-between mb-2 shrink-0 z-10">
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400 hidden sm:block">
            {isMobile ? "Tap to inspect vectors" : "Scroll or click to inspect vectors"}
          </span>
        </div>
      </div>

      {isMobile && (
        <MobileTabBar activeIndex={activeVector} onSelectVector={handleSelectVector} />
      )}
          ...
          >
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-bold text-teal-600 dark:text-cyan-400 flex items-center gap-1.5">
                <ArrowUpRight className="w-3.5 h-3.5" /> {currentVector.name}
              </span>
  ...
  return (
    <div
      ...
      className="relative h-auto lg:h-[400vh] w-full self-start"
    >
      <div className="lg:sticky lg:top-12 lg:max-h-[calc(100vh-64px)] w-full flex flex-col justify-between">
        {cardContent}
      </div>
    </div>
  );
